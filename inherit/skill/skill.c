...
// The following is to enable weapon with poison effect.
// Different poison types may be added later. By Ryu, 8/2/97
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
	object weapon = me->query_temp("weapon");
	
        if ( objectp(weapon) && weapon->query("poison_applied") > 0){
        victim->apply_condition("snake_poison",
       		random(weapon->query("poison_applied")
