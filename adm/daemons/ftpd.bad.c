 ...
 */
 // fixed problem in LIST  sdong, 08/04/98
 // enhanced security add ip checking, permission checking, etc.  sdong, 10/25/98
...
string ls2( string path, int column, int fd ) {
	string *files;
	int i, j, s;
	mixed *xfiles;
	mixed *stats;
...
static void data_conn( int fd, string mess, string name, int type ) {
	...
	socket_info[ new_fd ] = ([
		DATA	  : ((type == STRING && UTYPE != BINARY) ? replace_string(mess, "\n", "\r\n") : mess),
		POS	   : 0,
		PARENT_FD : fd,
		TYPE	  : type,
...
void data_close_callback( int fd ) {
	...
	if ( socket_info[ fd ][ TYPE ] == DOWNLOAD ) {
		...
		if ( bufferp(UDATA) ) data = read_buffer(UDATA, 0, sizeof(UDATA));
		else data = UDATA;

/*		do {
			if (catch( res = write_buffer( UPATH, pos, UDATA[pos..(pos+(BLOCK_SIZE-1))] ) ))
				socket_write( socket_info[ fd ][ PARENT_FD ],
...
*/
		if (catch( write_file( UPATH, data, 1 ) ))
			socket_write( socket_info[ fd ][ PARENT_FD ],
				  "452 Error writing file.\n");
...
static void parse_comm( int fd, string str ) {
	...
	switch( lower_case( command[ 0 ] ) )  {
		...
		case "user": // specify user name
			...
			// add ip checking and wiz level checking, sdong, 10/25/98
			if ( !check_access( command[ 1 ] ) || !wiz_level( command[ 1 ] )  ||
				!SECURITY_D->valid_wiz_login( command[ 1 ], socket_address(fd) ) ) {

				socket_write( fd, sprintf("530 User %s access denied from this ip %s.\n",
					  command[ 1 ], socket_address(fd) ) );
#ifdef LOG_TIME
				log_file( LOG_FILE, FTP_TIME );
#endif
...
#ifdef ANONYMOUS_FTP   // no more guest login, sdong 10/25/98
...
//			} else if ( !check_valid_read( tmp, fd ) )  {
			// enforce read access checking, sdong, 10/25/98
				...
				if ( tmp[(i-2)..(i-1)] == ".c" ||
					  tmp[(i-2)..(i-1)] == ".h" ) {
					tmp = read_file(tmp);
					data_conn( fd, tmp, command[ 1 ], STRING );
				} else {
			...
			if ((i = sizeof(command)) > 1 && command[1] == "-l") {
				...
				if ( check_valid_read( tmp, fd ) ) {
					...
					if( SECURITY_D->get_status( UNAME ) != "admin" &&
						 UNAME != "ryu" && UNAME != "qfy" && UNAME != "xuy" &&
						 UNAME != "cleansword" && UNAME != "sdong" ) tmp2 = ls2( tmp, 1, fd );
					 else tmp2 = ls( tmp, 1, fd );
		...
		case "list": // give list files in a directory
			...
			if ( check_valid_read( tmp, fd ) ) {
				if( SECURITY_D->get_status( UNAME ) != "admin" &&
					 UNAME != "ryu" && UNAME != "qfy" && UNAME != "xuy" && UNAME != "sdong" &&
					 UNAME != "cleansword"  )	tmp2 = ls2( tmp, 0, fd );
				else tmp2 = ls( tmp, 0, fd );
				if (tmp2 == "")
					socket_write( fd, "550 No files found.\n");
				else	data_conn( fd, tmp2, "ls", STRING );
			} else
				PERMISSION_DENIED550(command[ 1 ]);
			break;
