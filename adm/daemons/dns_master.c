...
//      if(previous_object()) return;
        ...
        // Don't handle request from unarthorized site
...
string query_mud_cname() { return INTERMUD_MUD_CNAME; }
...
string start_message()
{
        return sprintf( "||NAME:%s||CNAME:%s||VERSION:%s||MUDLIB:%s||HOST:%s||PORT:%d"
                "||PORTUDP:%d||TIME:%s||TCP:%s", Mud_name(), Mud_cname(),
