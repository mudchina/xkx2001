...
{
	if( me->query("subrace") == "爬虫" ) return combat_action_crawl[random(sizeof(combat_action_crawl))];
	if( me->query("subrace") == "飞虫" ) return combat_action_fly[random(sizeof(combat_action_fly))];
}
