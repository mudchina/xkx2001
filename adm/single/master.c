...
object connect()
{
	...
	if (err) {
		write("œ÷‘⁄”–»À’˝‘⁄–ﬁ∏ƒ π”√’ﬂ¡¨œﬂ≤ø∑›µƒ≥Ã Ω£¨«Î¥˝ª·‘Ÿ¿¥°£\n");
		write(err);
...
//	log_file("static/CRASHES", sprintf("last_alias: %s\n",
//		ALIAS_D->get_current_alias()));
}
...
void destruct_env_of(object ob)
{
	if (!interactive(ob))
		return;
	tell_object(ob, "ƒ„À˘¥Ê‘⁄µƒø’º‰±ªªŸ√¡À°£\n");
	ob->move(VOID_OB);
}
...
string standard_trace(mapping error, int caught)
{
    ...
    for (i=0, s = sizeof(error["trace"]); i < s; i++) {
		res = sprintf("%s∫ÙΩ–¿¥◊‘£∫%s µƒ %s() µ⁄ %i ––£¨ŒÔº˛£∫ %O\n",
			res,
