...
string *throw_msg = ({
HIY"$N拿起一枚金蛇锥，对准$n一扬手，将金蛇锥笔直射向$n的"+xuedao[random(sizeof(xuedao))]+NOR,
//"$N双手连挥，两枚金蛇锥带着呼啸风声，划成二弧射向$n的"+xuedao[random(sizeof(xuedao))]+"和"+xuedao[random(sizeof(xuedao))],
//"$N飞身跃起，居高临下向$n猛射三枚金蛇锥，直取$n的双目和"+xuedao[random(sizeof(xuedao))]+NOR,
HIY"突然$N低头恭身，只听得「嗖」的一声，一枚金蛇锥由后领飞出，往$n的"+xuedao[random(sizeof(xuedao))]+"电射而去"NOR,
HIY"但见$N将双手拢入袖中，无声无息地发出一枚金蛇锥，斜射到$n的"+xuedao[random(sizeof(xuedao))]+"，迅捷无比"NOR,
HIY"$N一笑而退，伸手却在腰间一拍，一枚金蛇锥由胸前猛然射出，飞取$n的"+xuedao[random(sizeof(xuedao))]+NOR,
...
int throw_ob(object me, object victim)
{
	...
	message_vision(action, me, victim);
	...
	if ( ap > dp){
        limbs = victim->query("limbs");
                ...
                message_vision(CYN"结果$N一声惨叫，$n已然插在$N的"+limbs[random(sizeof(limbs))]
                          +"上。\n"NOR, victim, ob);
                                 ob2=new(__FILE__);
                                 ob2->set_amount(1);
                                 ob2->set("embedded", 1);
