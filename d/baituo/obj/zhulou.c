...
void create()
{
	...
	if( clonep() )
		...
		set("unit", "个");
		set("long", "一个竹条编成的篓子，是白驼山庄的人专门用来捕蛇的。\n");
		set("material", "bamboo");
		set("value", 100);
	}
	setup();
...
int do_catch(string arg)
{
	...
	if( snake->query("who_breed") )
		return notify_fail("这条"+snake->name()+"是白驼山庄培养出来的，即使捉了，也不能进行育种。\n");
	...
	if(!snake->query("bt_snake")) 
		return notify_fail("这不是白驼山蛇谷的品种。\n");
	...
	if( snake->move(obj) ) 
	{
		...
		me->improve_skill( "training", snake->query("snake_poison") );
