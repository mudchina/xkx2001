...
int do_hit(string arg)
{
	...
	me->receive_damage("qi", 30, "被鼓声震死了");
...
int do_out(string arg)
{
        ...
        jingli_cost = 600 / (int)me->query("int");
        if(((int)me->query("jing") < ging_cost) || ((int)me->query("jingli") < jingli_cost))
                i = 0;
        ...
        if ( i < 50)
	{
		me->set_temp("last_damage_from", "从塔顶跳下来摔死了");
                ...
                if( i > 325 && me->query_skill("shaolin-shenfa", 1) <= 100   )
