...
#define _DES "一只小巧玲珑的丹炉，青铜质地，模样古旧，两侧各有一只兽口吞环，顶上\n一个盖子(lip)，盖中偏左有一孔，炉下设有一只炭盆，盆内的木炭(coal)\n看来是新加的。\n"
#define _FIRE HIR"\n丹炉下正燃着火。\n"NOR
#define _OPEN "丹炉盖子已经揭开。\n"

void create()
{
	set_name(CYN"丹炉"NOR,( {"dan lu","danlu","lu"} ));
	set_weight(3000);
	if (clonep() )
...
int do_open(string arg)
{
	...
	if (query("is_burning"))
	{
		message_vision(RED"只听「啊」的一声惨叫，原来$N被丹炉烫了手。\n"NOR,me);
		me->add("qi",-150+random(50),"被烫死了。");
		set("long",_DES+_FIRE);
		return 1;
	}
        message_vision("$N将丹炉盖子揭开。\n", me);
	set("long",_DES+_OPEN);
	set("open", 1);
	return 1;
}
...
int do_addyao(string arg)
{
	...
	if (item == "all" ) 
		return notify_fail("这小小丹炉怎么装得下这么多东西？\n");

	if (!objectp(obj = present(item, me)))
		return notify_fail("东西呢？\n");
                                                                
	if (!obj->query("yaocai"))
		return notify_fail("丹炉里只能放用以炼制的药材。\n");
	...
	message_vision("$N把一"+obj->query("unit")+obj->query("name")+"放进丹炉。\n",me);
	destruct(obj);
	add("count",-1);
...
int do_addwater(string arg)
{
	...
	set("water",1);
	message_vision("$N从"+ob->query("name")+"中倒了一些水在丹炉里。\n",me);
	return 1;
}
...
void burning(object me)
{
	...
	{
		set("long","一只小巧玲珑的炼丹炉。\n");
		message_vision(HIR"突然间丹炉内火光大盛，黑烟冒出，$N急忙将火头扑灭。\n"NOR,me);
...
if (wizardp(me))		
	...
	if (time == 0)
		time = 20;
	if (time > 30)
		me->start_busy(time-random(5));
	else
		me->start_busy((int)(time/2));
	set("pending", 1);
...
void finish(object me)		
{
	...
	&& (me->query("potential") < me->query("max_potential")) ) {
		me->add("potential",(int)(me->query("neili")/400));
		me->add("combat_exp",(int)(me->query("neili")/800));
	}
...
{	
	...
	{		
		...
		message_vision("$N从丹炉中取出一"+ob->query("unit")+ob->query("name")+"\n\n", me);
		ob->move(me);
	}
