...
void create()
{
   set_name(HIW"冰魄银针"NOR, ({"bingpo yinzhen", "zhen", "bingpo"}));
   if (clonep())
...
int throw_ob(object me, object victim)
{
   ...
   message_vision(HIW"$N左手在袖底轻轻一扬，一到银光，向$n激射而去。\n" NOR, me, victim);
   ...
   limbs = victim->query("limbs");
	...
	if (ap > dp && !victim->query_temp("armor/embed")){
                message_vision(CYN"\n结果$N一声惨叫，冰魄银针已然穿进了$N的"+limbs[random(sizeof(limbs))]
                          +"。\n"NOR, victim, ob);
                                 ob2=new(__FILE__);
                                 ob2->set_amount(1);
                                 ob2->set("embedded", 1);
	...
	if (victim->query("neili") > me->query("neili"))
        message_vision(RED"$n袍袖一拂，一股内劲发出，将那银针激得斜在一旁，插在地上。\n" NOR, me, victim);
