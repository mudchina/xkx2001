...
{	
	...
	::init();
	if( interactive(ob) && !is_fighting() && file_name(environment(this_object())) == "/d/dali/dalicheng10"   )
	{
...
{
	if( !ob || environment(ob) != environment() && file_name(environment(this_object())) == "/d/dali/dalicheng10" ) return;
	switch( random(2) )
	{
		case 0:
...
void kicking(object ob)
{
	...
	ob->move("/d/dali/dalicheng9");
	message("vision","只听“乒”地一声，" +  ob->query("name") + "被人从太和居里一脚踢了出来，狼狈万状的逃开了。\n", environment(ob), ob);
}
