...
private int shoot_result(object me, object victim, object ob)
{
	...
	if(!victim->query_temp("armor/embed")){
				 ...
				 power*ob->query("damage")+jiali*2, me);
					...
					+"上。\n"NOR, victim, ob);
		message_vision(HIR"你远远看到"+victim->query("name")+"一声惨叫，"+ob->query("name")+"已然插在"+victim->query("name")+"的"+limbs[random(sizeof(limbs))]+"上。\n"NOR,this_object());
	} else {
	//	ob2 = present("arrow", victim);
		...
		if ( ob2->query("weapon_type") == "arrow" ) {
			...
			message_vision(HIR"你远远看到"+victim->query("name")+"闷哼一声，"+ob->query("name")+"从"+victim->query("name")+"的"+limbs[random(sizeof(limbs))]+"擦过，刮下一大片皮肉，鲜血淋漓！\n"NOR,this_object());
		}
		...
		victim->receive_wound("qi",
		  ...
		  message_vision(HIR"你远远看到("+victim->query("name")+result+")\n",this_object());
	...
	if( victim->query("combat_exp") >= me->query("combat_exp") * 2/3 )
		...
		message_vision(HIR"你远远看到"+victim->query("name")+"不料眼明手快，身子躲过一侧。\n"NOR,this_object());
      }
        return 1;
}
