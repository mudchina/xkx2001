...
，四周十二张镶银象牙扶手红木椅，杯碗勺箸，入眼生辉。这里便是醉仙楼的玫
瑰厅，桌上平铺着一份菜单(menu)。
LONG );
        set("exits", ([ /* sizeof() == 1 */
            "west" : __DIR__"datang",
...
int do_broadcast(string arg)
{
        ...
        if (type == "marry") {
                mesg = me->name() + "现在在醉仙楼玫瑰厅正在举行订婚酒宴！\n";
        } else if (type == "birthday") {
                mesg = me->name() + "现在在醉仙楼玫瑰厅正在举行生辰寿宴！\n";
        } else if (type == "player") {
...
int do_order(string arg)
{
        ...
        if (arg == "marry") {
                ...
                create_food(RED"蟹黄豆腐"NOR, ({ "doufu","dish" }) );
                create_food(HIR"油煎螃蟹"NOR, ({ "crab" }) );
                create_food(HIY"粉蒸肉"NOR, ({ "meat" }) );
                create_food(YEL"兰花鲍鱼"NOR, ({ "baoyu","fish","lanhua baoyu" }) );
                ...
                create_food(HIR"蟹黄豆腐"NOR, ({ "doufu","dish" }) );
                create_food(HIY"北京烤鸭"NOR, ({ "duck" }) );
                create_food(YEL"粉蒸肉"NOR, ({ "meat" }) );
                create_food(HIR"扒烧猪头"NOR, ({ "pork","meat","pighead" }) );
        } else if (arg == "player") {
                mesg = me->name() + "现在在醉仙楼玫瑰厅宴请各路英雄豪杰！\n";
                ...
                create_food(HIY"德州扒鸡"NOR, ({ "chicken" }) );
                create_food(RED"菊花海螺"NOR, ({ "hailuo","dish" }) );
                create_food(HIY"北京烤鸭"NOR, ({ "duck" }) );
                create_food(HIW"白斩鸡"NOR, ({ "chicken" }) );
                create_food(GRN"竹叶蒸鸡"NOR, ({ "chicken" }) );
                create_food(HIR"蟹黄豆腐"NOR, ({ "doufu","dish" }) );
                create_food(HIR"麻婆豆腐"NOR, ({ "bean curd" }) );
                create_food(RED"蚝油肉片"NOR, ({"fish meat","meat"}) );
                create_food(HIW"马蹄桂鱼"NOR, ({"fish"}) );
                create_food(HIW"麻辣手撕鸡"NOR, ({"chicken"}) );
                create_food(HIR"干煸尤鱼"NOR, ({"fish"}) );
                create_food(HIY"东北水饺"NOR, ({ "dumpling" }) );
