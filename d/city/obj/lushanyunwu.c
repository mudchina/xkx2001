...
void create()
{
        set_name(WHT"庐山云雾"NOR,({ "lushan yunwu", "tea", "cha" }));
        set_weight(50);
        if (clonep())
                set_default_object(__FILE__);
        else {
                set("long", "庐山云雾产于江西庐山，是绿茶中的精品。\n");
                set("unit", "杯");
                set("value", 30);
        set("remaining", 3);
        set("drink_supply", 30);
...
int do_drink(string arg)
{
    ...
    add("remaining", -1);
        if ( query("remaining") )
        {
                 message_vision("$N端起茶杯，有滋有味地品了几口"WHT"庐山云雾"NOR"。\n"+
                "一股香气直入心脾，$N觉得精神好多了。\n", this_player());
        } else 
        { 
                 message_vision("$N端起茶杯，把剩下的"WHT"庐山云雾"NOR"一饮而尽。\n"+
                "一股香气直入心脾，$N觉得精神好多了。\n", this_player());
