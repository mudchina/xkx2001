...
void create()
{
        set_name(WHT"老君眉"NOR,({ "laojun mei", "tea", "cha" }));
        set_weight(50);
        if (clonep())
                set_default_object(__FILE__);
        else {
                set("long", "三起三落，雀舌含珠，刀丛林立，叹为观止。\n");
                set("unit", "杯");
                set("value", 30);
        set("remaining", 3);
        set("drink_supply", 30);
...
int do_drink(string arg)
{
    ...
    add("remaining", -1);
        if ( query("remaining") )
        {
                 message_vision("$N端起茶杯，有滋有味地品了几口"WHT"老君眉"NOR"。\n"+
                "一股香气直入心脾，$N觉得精神好多了。\n", this_player());
        } else 
        { 
                 message_vision("$N端起茶杯，把剩下的"WHT"老君眉"NOR"一饮而尽。\n"+
                "一股香气直入心脾，$N觉得精神好多了。\n", this_player());
