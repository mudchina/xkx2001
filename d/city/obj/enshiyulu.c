...
void create()
{
        set_name(HIG"恩施玉露"NOR,({ "enshi yulu", "tea", "cha" }));
        set_weight(50);
        if (clonep())
                set_default_object(__FILE__);
        else {
                set("long", "一杯香气清鲜，滋味甘醇，湛绿如玉的湖北名茗－－恩施玉露。\n");
                set("unit", "杯");
                set("value", 30);
        set("remaining", 3);
        set("drink_supply", 30);
...
int do_drink(string arg)
{
    ...
    add("remaining", -1);
        if ( query("remaining") )
        {
                 message_vision("$N端起茶杯，有滋有味地品了几口"HIG"恩施玉露"NOR"。\n"+
                "一股香气直入心脾，$N觉得精神好多了。\n", this_player());
        } else 
        { 
                 message_vision("$N端起茶杯，把剩下的"HIG"恩施玉露"NOR"一饮而尽。\n"+
                "一股香气直入心脾，$N觉得精神好多了。\n", this_player());
