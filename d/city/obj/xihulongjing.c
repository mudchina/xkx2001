...
void create()
{
        set_name(BGRN+BLK"西湖龙井"NOR,({ "xihu longjing", "tea", "cha" }));
        set_weight(50);
        if (clonep())
                set_default_object(__FILE__);
        else {
                set("long", "色翠、香郁、味醇、形美，茶叶在杯中逐渐伸展，\n一旗一枪，汤明色绿，历历在目。
");
                set("unit", "杯");
                set("value", 30);
        set("remaining", 3);
...
int do_drink(string arg)
{
    ...
    add("remaining", -1);
        if ( query("remaining") )
        {
                 message_vision("$N端起茶杯，有滋有味地品了几口"BGRN+BLK"西湖龙井"NOR"。\n"+
                "一股香气直入心脾，$N觉得精神好多了。\n", this_player());
        } else 
        { 
                 message_vision("$N端起茶杯，把剩下的"BGRN+BLK"西湖龙井"NOR"一饮而尽。\n"+
                "一股香气直入心脾，$N觉得精神好多了。\n", this_player());
