...
int do_hold(string arg)
{
	...
	if( ob->query("race") == "蛇类" || ob->query("race")=="昆虫")
		...
		else message_vision(GRN"$N猛的出手用蓝玉钵扣向"+ob->query("name")+",但"+ob->query("name")+"机灵的跳开了。\n"NOR,this_player());
		
		return 1;
		}
	else return notify_fail("这个"+ob->query("name")+"恐怕不能做成毒药粉！\n");
}
...
{       string what;
	...
	if (ob->query("poison_amount") && this_player()->query_skill("poison", 1) < ob->query("poison_amount"))
		return notify_fail(ob->query("name")+"上毒性已经很强了，你还是不要再布毒了吧。\n");

	message_vision(RED"$N小心翼翼地把一些毒药布在" +ob->query("name")+"上。\n"NOR, this_player());
